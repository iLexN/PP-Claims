...
return{'disable':disableBtn,'enable':enableBtn};})();function isOnLine(e){if(navigator.onLine){console.log('online');}else{e.preventDefault();console.log('offline');}}
$('form').on({'submit':isOnLine});window.addEventListener('online',isOnLine);window.addEventListener('offline',isOnLine);function debounce(func,wait,immediate){var timeout;return function(){var context=this,args=arguments;var later=function(){timeout=null;if(!immediate)
func.apply(context,args);};var callNow=immediate&&!timeout;clearTimeout(timeout);timeout=setTimeout(later,wait);if(callNow)
func.apply(context,args);};}
$.extend({jshook:function(hookName){var selector;if(!hookName||hookName==='*'){selector='[data-jshook]';}else{selector='[data-jshook~="'+hookName+'"]';}
return $(selector);}});